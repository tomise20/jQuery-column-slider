...
function desktopSlider() {
    ...
    var timer = setInterval(() => { playAnimation("left", "left", ".slider", "right") }, sliderAutoplayTime);
    ...
    $(".slider").click(function () {
        ...
        timer = setInterval(() => { playAnimation("left", "left", ".slider", "right") }, sliderAutoplayTime);
    });
}
...
function mobileSlider() {
    ...
    var timer = setInterval(() => { playAnimation("top", "top", ".m-slider", "bottom") }, sliderAutoplayTime);
    ...
    $(".m-slider").click(function () {
        ...
        timer = setInterval(() => { playAnimation("top", "top", ".m-slider", "bottom") }, sliderAutoplayTime);
    });
}
...
function playAnimation(sliderDirection, animateDirection, sliderClass, secondaryDirection) {
    ...
    if (direction === sliderDirection) {
        ...
        if(sliderDirection == "left") {
            position = (sliderWidth - ((slideCount - (slideIndex - 1)) * 50)) + "px";
        } else {
    ...
    $(`${sliderClass}:nth(${slideIndex})`).stop().animate(animationParams, sliderSpeed, function () {
